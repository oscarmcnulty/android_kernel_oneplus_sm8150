...
#ifdef OPLUS_FEATURE_CHG_BASIC
static int __usb_temp_l_dbg = 0;
static int __usb_temp_r_dbg = 0;
...
module_param_named(
	usb_temp_l_dbg, __usb_temp_l_dbg, int, 0600
);
module_param_named(
	usb_temp_r_dbg, __usb_temp_r_dbg, int, 0600
);
#endif
...
int oplus_usbtemp_adc_gpio_init(struct oplus_chg_chip *chip)
{
	...
	chg->usbtemp_gpio12_default = pinctrl_lookup_state(chg->usbtemp_gpio12_adc_pinctrl, "gpio12_adc_default");
...
static void get_usb_temp(struct oplus_chg_chip *chg)
{
	...
	if(usbtemp_debug & TEST_FUNC_BIT){
		if (__usb_temp_l_dbg > 0)
			chg->usb_temp_l = __usb_temp_l_dbg;
		else
			chg->usb_temp_l = 60;

		if (__usb_temp_r_dbg > 0)
			chg->usb_temp_r = __usb_temp_r_dbg;
		else
			chg->usb_temp_r = 60;
	}
...
#ifdef OPLUS_CUSTOM_OP_DEF
	rc = smblib_get_iio_channel(chg, "skin_therm", &chg->iio.skin_temp_chan);
#else
...
//18115 return;
	...
	} else if (chip->batt_volt < 4000){ // 9v
...
#endif
	.get_skin_temp = smbchg_get_skin_temp,
};
#endif /* OPLUS_FEATURE_CHG_BASIC */
